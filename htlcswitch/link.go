...
func (l *channelLink) syncChanStates() error {
	...
	for _, htlc := range activeHTLCs {
		...
		// TODO(conner): Get source ref?
		err := l.channel.SettleHTLC(p, htlc.HtlcIndex, nil, nil)
		if err != nil {
...
func (l *channelLink) handleDownStreamPkt(pkt *htlcPacket, isReProcess bool) {
	...
	case *lnwire.UpdateFulfillHTLC:
		...
		// state machine.
		err := l.channel.SettleHTLC(htlc.PaymentPreimage,
			pkt.incomingHTLCID, pkt.sourceRef, pkt.destRef)
		if err != nil {
